#!/usr/bin/env python3
"""Summarize a Q2 preference study's choices, reasoning effects, and order bias."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--root",
        type=Path,
        default=Path("q2/results/qwen-preference-reasoning-depth"),
    )
    args = parser.parse_args()
    raw_path = args.root / "raw-results.json"
    summary_path = args.root / "summary.json"
    rates_path = args.root / "preference-rates.csv"
    transitions_path = args.root / "condition-transitions.csv"
    ...
    for domain in DOMAINS + ["all"]:
        for condition in CONDITIONS:
            ...
            costs = [row.get("cost") for row in subset if row.get("cost") is not None]
    ...
    summary = {
        ...
        "total_cost": sum(row.get("cost") or 0 for row in rows) if any(row.get("cost") is not None for row in rows) else None,
    ...
    with rates_path.open("w", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=list(rates[0]))
        writer.writeheader()
        writer.writerows(rates)
    with transitions_path.open("w", newline="") as file:
